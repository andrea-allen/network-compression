...
class MDL_populations():
    ...
    def __init__(self, edgesets, N, K0=1, n_fails=100, bipartite=None, directed=False, max_runs=np.inf):
        ...
        if bipartite is not None:
            self.NC2 = bipartite[0] * bipartite[
                1]  # bipartite networks only differentiated from unipartite ones through this term
        if directed:
            self.NC2 = self.N * (
                        self.N - 1)  # directed networks only differentiated from undirected ones through this term
        else:
    ...
    def update_mode(self, Ek, Sk):
        ...
        if (Sk == 1):
            return Acomplete  # return network itself if cluster only has one network
        elif not (Ek):
            return set()  # return empty mode if networks in cluster are empty
        elif (len(Ek_vals) == 1) and (next(iter(Ek_vals)) > 1):
            return Acomplete  # return network itself if networks are all duplicates
        ...
        while (r < Mmax):
            ...
            e, Xij = Etil[r]
            Lafter = self.logchoose(self.NC2, Mk - 1) + Sk * np.log(self.S / Sk) + self.logchoose(Sk * (Mk - 1),
                                                                                                  tk - Xij) \
                     + self.logchoose(Sk * (self.NC2 - (Mk - 1)), fk + Xij)
            Lbefore = self.logchoose(self.NC2, Mk) + Sk * np.log(self.S / Sk) + self.logchoose(Sk * Mk,
                                                                                               tk) + self.logchoose(
                Sk * (self.NC2 - Mk), fk)
    ...
    def Lk(self, Ak, Ek, Sk):
        ...
        for e in Ak:
            if e in Ek:
                tk += Ek[e]
                fk -= Ek[e]
        return self.logchoose(self.NC2, Mk) + Sk * np.log(self.S / Sk) + self.logchoose(Sk * Mk, tk) + self.logchoose(
            Sk * (self.NC2 - Mk), fk)
    ...
    def move2(self):
        ...
        if ((kp, kpp) in self.attmerges) or (
                (kpp, kp) in self.attmerges):  # check if merge already has been tried and failed
            return False, 0
    ...
    def move3(self):
        ...
        deltaL3 = self.Lk(localA[0], localE[0], localS[0]) + self.Lk(localA[1], localE[1], localS[1]) - self.Lk(
            self.A[k], self.E[k], Sk)
    ...
    def move4(self):
        ...
        if ((k1, k2) in self.attmergesplits) or (
                (k2, k1) in self.attmergesplits):  # check if merge-split combo already tried with these clusters
            return False, 0
    ...
    def run_sims(self):
        ...
        self.L = sum([self.Lk(self.A[k], self.E[k], len(self.C[k])) for k in self.C])
        self.L /= self.logchoose(self.S * self.NC2,
                                 M)  # return (minimum description length)/(naive code length transmitting all networks separately)
    ...
    def dynamic_contiguous(self):
        ...
        for j in range(1, self.S):
            ...
            for i in np.arange(j - 1, -1, -1):
                ...
                Lprop += self.Lk(Aprop[jkey], localE, len(Cprop[jkey])) - len(Cprop[jkey]) * np.log(
                    self.S / len(Cprop[jkey])) + np.log(self.S)
